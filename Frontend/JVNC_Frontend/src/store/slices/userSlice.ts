...
const userSlice = createSlice({
  ...
  reducers: {
    ...
    fetchUsersRequest: (state, action: PayloadAction<{ page?: number; size?: number; search?: string }>) => {
      state.isLoading = true;
      state.error = null;
    },
    fetchUsersSuccess: (state, action: PayloadAction<{ users: User[]; total: number; page: number }>) => {
    ...
    // Update user actions
    updateUserRequest: (state, action: PayloadAction<{ id: number; user: Partial<User> }>) => {
      state.isLoading = true;
      state.error = null;
    },
